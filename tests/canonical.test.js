...
const inst = (key, value, observedAt) => ({
    instanceKey: key,
...
test('an explicit instanceKey selects that instance, not the newest', () => {
    ...
    assert.equal(resolveCanonicalValue(v, {
...
test('no instanceKey means latest by observedAt — v1 flat-mirror behaviour', () => {
    ...
    assert.equal(resolveCanonicalValue(v, { category: 'applicant-contact', field: 'contactValue' }), 'newer');
});
test('an undated instance never displaces a dated one', () => {
    ...
    assert.equal(resolveCanonicalValue(v, { category: 'applicant-contact', field: 'contactValue' }), 'dated');
});
test('an undated instance is still returned when it is all there is', () => {
    const v = view([inst('b', 'undated')]);
    assert.equal(resolveCanonicalValue(v, { category: 'applicant-contact', field: 'contactValue' }), 'undated');
});
test('instances that lack the requested field are not candidates', () => {
    ...
    assert.equal(resolveCanonicalValue(v, { category: 'applicant-contact', field: 'contactValue' }), '+60111111111');
});
